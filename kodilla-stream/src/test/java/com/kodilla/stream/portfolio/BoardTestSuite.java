...
public class BoardTestSuite {
    ...
    public void testAddTaskListFindUsersTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()
                ...
                .filter(t -> t.getAssigneUser().equals(user))
                .collect(toList());
        //Then
        Assert.assertEquals(2, tasks.size());
        Assert.assertEquals(user, tasks.get(0).getAssigneUser());
        Assert.assertEquals(user, tasks.get(1).getAssigneUser());
    }
    @Test
    ...
    @Test
    public void testtestAddTaskListAverageWorkingOnTask() {
        //Given
        Board project = prepareTestData();
        //When
        List<TaskList> inProgressTasks = new ArrayList<>();
